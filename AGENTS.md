...
- **KEEP IT SIMPLE.** Prefer the most direct expression of an idea over elaborate helper
  ...
  a non-exported function until it is used at least twice. Do not be too fussy about
  argument checking: check inputs to the api, but not to unexported functions. Let
  base R catch errors, unless an error would silently give a misleading result.
...
- **BLUNDER REVIEW.** For non-trivial changes, and always for numerical,
  ...
  changes, do a separate adversarial review before committing: spawn a subagent
  for the review, and ask only for severe mistakes such as duplicated expensive 
  work, wrong abstraction layer, missed existing APIs, unnecessary helpers, 
  changed semantics, hidden broad costs, or untested edge cases; address real 
  blockers before committing, or explicitly explain why they are not blockers.
