...
alternative tested. Numbers show `RmlxStats time/alternative time`. 
...
base_comparison$speedup <- 100 *
  base_comparison$mlxs_time/base_comparison$base_time
...
fastest_comparison$speedup <- 100 *
  fastest_comparison$mlxs_time/fastest_comparison$fastest_time
...
huxtable::as_hux(base_table, add_rownames = "") |> 
  huxtable::set_number_format(-1, -1, 1) |> 
  huxtable::set_font_size(11) |> 
  huxtable::set_caption(
    "RmlxStats time vs base R (%). Below 100% → RmlxStats is faster")|> 
  huxtable::set_header_cols(1, TRUE) |> 
  huxtable::style_headers(bold = TRUE) |> 
  huxtable::map_text_color(-1, -1,
                                 huxtable::by_colorspace("green3", "grey40", "red3")) |> 
  huxtable::restack_down(cols = 7) |> 
  huxtable::set_bottom_border(5, -1)

huxtable::as_hux(fastest_table, add_rownames = "") |> 
  huxtable::set_font_size(11) |> 
  huxtable::set_number_format(-1, -1, 1) |> 
  huxtable::set_caption(
    "RmlxStats time vs fastest alternative (%). Below 100% → RmlxStats is faster") |> 
  huxtable::set_header_cols(1, TRUE) |> 
  huxtable::style_headers(bold = TRUE)|> 
  huxtable::map_text_color(-1, -1, 
                                 huxtable::by_colorspace("green3", "grey40", "red3")) |> 
