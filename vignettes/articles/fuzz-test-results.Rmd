...
fuzz |> 
  ...
  mutate(
    model = stringr::str_extract(scenario, "(.*?)_", group = 1),
    model = ifelse(model == "cv", "cv_glmnet", model),
    Scenario = stringr::str_extract(scenario, "(.*?)_(.*)", group = 2),
    Scenario = stringr::str_replace_all(Scenario, "_", " "),
    Scenario = stringr::str_replace_all(Scenario, "glmnet ", "")
  ) |> 
  ggplot(aes(y = value, x = datetime_utc, colour = Scenario, 
             shape = interaction(tier, branch))) + 
    geom_point() + 
    geom_line(aes(group = interaction(Scenario, tier, branch), 
                  linetype = interaction(tier, branch))) +
    scale_y_log10() + 
...
```{r, fig.height = 20, eval = nrow(fuzz_glm_det) > 0}
...
the true d.g.p. (i.e. if you know that, how well do you predict the test set?)
We compare glmnet::glmnet as a reference.
...
We also check values of our objective function (within the training set). If
we do worse than glmnet (positive values of e.g. 0.1 or more) that suggests
we should check performance of the algorithm - we're not maximizing 
what we are trying to maximize.
...
We compare `mlxs_cv_glmnet()` to the reference `glmnet::cv.glmnet()`. Errors
are:

* `out_of_fold_prediction`: maximum absolute difference for out-of-fold predictions
* `cv_loss`: maximum absolute difference of `cvm`, the cross-validated error
  for each value of lambda
* `cv_loss_se`: maximum absolute difference of `cvsd`, the estimated s.e.
  of `cvm`
