...
n_sizes <- c(10000, 50000, 250000) / 10
...
for (i in seq_len(nrow(glmnet_grid))) {
  ...
  stats_time <- system.time({
    fit_stats <- glmnet::glmnet(x, y_sparse_subset, lambda = lambda_grid)
    as.numeric(coef(fit_stats)[1, 1])
  })[["elapsed"]]
  mlx_time <- system.time({
    fit_mlx <- mlxs_glmnet(x, y_sparse_subset, lambda = lambda_grid)
    Rmlx::mlx_eval(fit_mlx$x_center)
    Rmlx::mlx_eval(fit_mlx$x_scale)
    as.numeric(fit_mlx$a0[1])
  })[["elapsed"]]

  bm <- data.frame(
    expression = c("glmnet::glmnet", "RmlxStats::mlxs_glmnet"),
  ...
  fits <- list(
    "glmnet::glmnet" = fit_stats,
    "RmlxStats::mlxs_glmnet" = fit_mlx
  )
  scores <- lapply(names(fits), function(method) {
    beta_path <- as.matrix(coef(fits[[method]]))[-1, , drop = FALSE]
    data.frame(
      model_type = "glmnet",
      n = n,
      p = p,
      method = method,
      accuracy = safe_relative_rmse(
        beta_path[, lambda_accuracy_idx],
        beta_target
      ),
      stringsAsFactors = FALSE
    )
...
boot_accuracy <- list()
bootstrap_reps <- if (params$develop) 50L else 200L
oracle_reps <- if (params$develop) 50L else 300L
...
for (i in seq_len(nrow(boot_grid))) {
  ...
  fitters <- list(
    "stats::lm" = function() summary(lm(boot_formula, data = subset_data))$coef[, 2],
...
Errors are calculated as the relative root mean squared error of the estimated betas, for a fixed value of `lambda`.
