...
We compare truncated PCA against `stats::prcomp()`, asking both methods for the
leading 10 components. This keeps the benchmark focused on the regime where the
MLX-backed randomized path is intended to help.

```{r pca-benchmarks}
pca_results <- list()
pca_grid <- bench_grid[bench_grid$n <= 50000 & bench_grid$p <= 400, ]
